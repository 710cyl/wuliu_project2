...
namespace Demo1._1._3
{
        ...
        {
            string msg = null;
            using (var ws = new WebSocket("ws://" + IP + ":" + Host + "/ServerTransportationRegister"))
            {
                ws.Connect();
                ws.Send(Transpot_ID);
                while (msg == null)
        ...
        {
            string msg = null;
            using (var ws = new WebSocket("ws://" + IP + ":" + Host + "/ServerFleetPrice"))
            {
                ws.Connect();
                ws.Send(Price_ID);
                while (msg == null)
        ...
        {
            string msg = null;
            using (var ws = new WebSocket("ws://" + IP + ":" + Host + "/ServerShipperPrice"))
            {
                ws.Connect();
                ws.Send(Price_ID);
                while (msg == null)
